...
export module yspeech.runtime.stage_path_executor;
...
export class StagePathExecutor {
public:
...
public:
    ...
    void configure(PipelineBuilderConfig config, RuntimeContext& runtime, SegmentRegistry& registry) {
        ...
        if (running) {
            throw std::runtime_error("cannot reconfigure stage path executor while running");
        }
    ...
    void set_stage_callback(PipelineStageRole role, StagePathExecutor::StageCallback callback) {
        std::scoped_lock lock(state_mutex);
        stage_callbacks[role] = std::move(callback);
    }

    void set_completion_callback(StagePathExecutor::CompletionCallback callback) {
        std::scoped_lock lock(state_mutex);
    ...
    void start() {
        ...
        if (!runtime_context || !segment_registry) {
            throw std::runtime_error("stage path executor is not configured");
        }
        ...
        running = true;
        log_info("Stage path executor started with {} lines", builder_config.num_lines);
    }
    ...
    void wait() {
        ...
        line_tokens.clear();
        running = false;
        log_info("Stage path executor stopped");
    }
...
private:
    ...
    auto snapshot_callback(PipelineStageRole role) -> StagePathExecutor::StageCallback {
        std::scoped_lock lock(state_mutex);
    ...
    auto snapshot_completion_callback() -> StagePathExecutor::CompletionCallback {
        std::scoped_lock lock(state_mutex);
    ...
    std::unordered_map<PipelineStageRole, StagePathExecutor::StageCallback> stage_callbacks;
    StagePathExecutor::CompletionCallback completion_callback;
...
StagePathExecutor::StagePathExecutor()
...
void StagePathExecutor::configure(PipelineBuilderConfig config, RuntimeContext& runtime, SegmentRegistry& registry) {
    impl_->configure(std::move(config), runtime, registry);
}

void StagePathExecutor::set_stage_callback(PipelineStageRole role, StageCallback callback) {
    impl_->set_stage_callback(role, std::move(callback));
}

void StagePathExecutor::set_completion_callback(CompletionCallback callback) {
    impl_->set_completion_callback(std::move(callback));
}

void StagePathExecutor::start() {
    impl_->start();
}

void StagePathExecutor::push(PipelineToken token) {
    impl_->push(std::move(token));
}

void StagePathExecutor::finish() {
    impl_->finish();
}

void StagePathExecutor::stop() {
    impl_->stop();
}

void StagePathExecutor::wait() {
    impl_->wait();
}

bool StagePathExecutor::is_running() const {
    return impl_->is_running();
}

auto StagePathExecutor::pending_tokens() const -> std::size_t {
