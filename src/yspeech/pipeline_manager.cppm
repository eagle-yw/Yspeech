...
public:
    ...
    void build() {
        ...
        size_t concurrency = config_.max_concurrency();
        executor_ = std::make_unique<tf::Executor>(concurrency);
        log_debug("PipelineStage '{}' using executor with {} threads", 
                 config_.id().empty() ? "unnamed" : config_.id(), concurrency);
...
private:
    ...
    void execute(Context& ctx, StreamStore* store, bool flush) {
        ...
        if (executor_) { executor_->run(taskflow_).wait(); }
