...
public:
    static std::string to_json(const ProcessingStats& stats) {
        ...
        j["core_total_time_ms"] = stats.core_total_time_ms;
        j["core_active_time_ms"] = stats.core_active_time_ms;
        j["non_core_time_ms"] = stats.non_core_time_ms;
        j["core_time_percent"] = stats.core_time_percent;
        j["non_core_time_percent"] = stats.non_core_time_percent;
        ...
        if (!stats.core_timings.empty()) {
            nlohmann::json timings_json = nlohmann::json::array();
            for (const auto& [id, timing] : stats.core_timings) {
                nlohmann::json t;
    ...
    static std::string to_csv(const ProcessingStats& stats) {
        ...
        oss << "core_total_time_ms," << std::fixed << std::setprecision(2) << stats.core_total_time_ms << "\n";
        oss << "core_active_time_ms," << std::fixed << std::setprecision(2) << stats.core_active_time_ms << "\n";
        oss << "non_core_time_ms," << std::fixed << std::setprecision(2) << stats.non_core_time_ms << "\n";
        oss << "core_time_percent," << std::fixed << std::setprecision(2) << stats.core_time_percent << "\n";
        oss << "non_core_time_percent," << std::fixed << std::setprecision(2) << stats.non_core_time_percent << "\n";
        ...
        if (!stats.core_timings.empty()) {
            oss << "\ncore_id,total_time_ms,active_wall_time_ms,task_time_percent,avg_time_ms,min_time_ms,max_time_ms,call_count,effective_call_count,effective_avg_time_ms,p50_ms,p95_ms,p99_ms\n";
            for (const auto& [id, timing] : stats.core_timings) {
                const double task_time_percent =
                    stats.total_processing_time_ms > 0.0
                        ? ((timing.active_wall_time_ms > 0.0
